...
`Generate Crypto Key (Ed25519)` Generates a new `Ed25519` keypair. This function requires no inputs,
there is however an optional input of `Password` <Step text="1"/> : String, a pass-phrase used to encrypt the generated key.
...
there is however two optional inputs:

* `Key Size` <Step text="1"/> : Integer, size of the `RSA` key (Set to 2048 by default).
* `Password` <Step text="2"/> : String, a pass-phrase used to encrypt the generated key.
